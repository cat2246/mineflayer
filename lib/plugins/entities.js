...
function inject (bot) {
  ...
  function packetVelocityUsesLpVec3 (packetType) {
    return bot.registry.protocol.play.toClient.types[packetType][1]
      .some(field => field.name === 'velocity' && field.type === 'lpVec3')
  }

  function fromPacketVelocity (packetType, velocity) {
    const packetVelocity = new Vec3(velocity.x, velocity.y, velocity.z)
    return packetVelocityUsesLpVec3(packetType) ? packetVelocity : conv.fromNotchVelocity(packetVelocity)
  }
  ...
  bot._client.on('spawn_entity', (packet) => {
    ...
    if (packet.velocity !== undefined) {
      entity.velocity.update(fromPacketVelocity('packet_spawn_entity', packet.velocity))
    }
    bot.emit('entitySpawn', entity)
  })
  ...
  bot._client.on('spawn_entity_living', (packet) => {
    ...
    entity.velocity.update(fromPacketVelocity('packet_spawn_entity_living', packet.velocity))
  ...
  bot._client.on('entity_velocity', (packet) => {
    // entity velocity
    const entity = fetchEntity(packet.entityId)
    entity.velocity.update(fromPacketVelocity('packet_entity_velocity', packet.velocity))
  })
  ...
  bot._client.on('damage_event', (packet) => { // 1.20+
    ...
    const directSource = bot.entities[packet.sourceDirectId - 1]
    bot.emit('entityHurt', entity, source, {
      ...
      sourceDirectPosition: vectorData(directSource?.position),
      sourceDirectVelocity: vectorData(directSource?.velocity)
    })
  })
